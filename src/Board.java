...
public class Board extends JComponent
{
      ...
      addMouseListener(new MouseAdapter()
                       {
                          ...
                          public void mouseReleased(MouseEvent me)
                          {
                        	 ...
                        	 if(Math.abs(posCheck.cx-oldcx) != 62 || Math.abs(posCheck.cy-oldcy) != 62){
                                 Board.this.posCheck.cx = oldcx;
                                 Board.this.posCheck.cy = oldcy;
                        	 }
                             if(posCheck.checker.checkerType==CheckerType.BLACK_REGULAR){
                            	 
                            	 if(Math.abs(posCheck.cx-oldcx) != 62 || (posCheck.cy-oldcy) != 62){
                                     Board.this.posCheck.cx = oldcx;
                                     Board.this.posCheck.cy = oldcy;
                            	 }
                             }
                             if(posCheck.checker.checkerType == CheckerType.RED_REGULAR){
                            	 if(Math.abs(posCheck.cx-oldcx) != 62 || (posCheck.cy-oldcy) != -62){
                                     Board.this.posCheck.cx = oldcx;
                                     Board.this.posCheck.cy = oldcy;
                            	 }
                             ...
                             int oldcol = (int) Math.ceil((oldcx*2.0-SQUAREDIM)/(2.0*SQUAREDIM));
                             int oldrow = (int) Math.ceil((oldcy*2.0-SQUAREDIM)/(2.0*SQUAREDIM));
                             
                             int newcol= (int) Math.ceil((posCheck.cx*2.0-SQUAREDIM)/(2.0*SQUAREDIM));
                             int newrow = (int) Math.ceil((posCheck.cy*2.0-SQUAREDIM)/(2.0*SQUAREDIM));

                             b[oldrow][oldcol]='_';
                             b[newrow][newcol]=(posCheck.checker.checkerType==CheckerType.BLACK_REGULAR)? 'x':'o';
   ...
   public void add(Checker checker, int row, int col)
   {
      ...
      b[row-1][col-1]=(checker.checkerType==CheckerType.BLACK_REGULAR)? 'x':'o';
   }
